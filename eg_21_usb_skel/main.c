...
static struct usb_device_id skel_table[] = { { USB_DEVICE(USB_VENDOR_ID,
							  USB_PRODUCT_ID) },
					     {} };
...
static void skel_delete(struct kref *kref)
{
	...
	if (dev->int_in_buffer)
		usb_free_coherent(dev->udev, 8, dev->int_in_buffer,
				  dev->irq_urb->transfer_dma);
			  ...
			  size_t count, loff_t *ppos)
{
	...
	urb_buf = usb_alloc_coherent(dev->udev, sizeof(val), GFP_KERNEL,
				     &urb->transfer_dma);
	if (!urb_buf) {
		retval = -ENOMEM;
		goto error;
		      ...
		      const struct usb_device_id *id)
{
	...
	dev->int_in_buffer = usb_alloc_coherent(dev->udev, 8, GFP_ATOMIC,
						&dev->irq_urb->transfer_dma);
	if (!dev->int_in_buffer) {
		retval = -ENOMEM;
