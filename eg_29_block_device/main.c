...
static void block_release(struct gendisk *gdisk, fmode_t mode)
{
...
static int block_media_changed(struct gendisk *gdisk)
{
	struct ldd_dev *ldd_dev = gdisk->private_data;
...
static int block_revalidate(struct gendisk *gdisk)
{
	struct ldd_dev *ldd_dev = gdisk->private_data;
...
static void transfer(struct ldd_dev *ldd_dev, unsigned long sector,
...
static blk_status_t ldd_block_request_simple(struct blk_mq_hw_ctx *hctx,
...
static int block_xfer_bio(struct ldd_dev *ldd_dev, struct bio *bio)
{
	struct bio_vec bvec;
...
static int block_xfer_request(struct ldd_dev *ldd_dev, struct request *req)
{
	struct bio *bio;
	int nsect = 0;
